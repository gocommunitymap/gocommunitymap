...
import { getHouseRule, updateHouseRule, deleteHouseRule } from 'src/store'
...
const HouseRule = () => {
  ...
  const store = useSelector(state => state.houseRule)
  ...
  useEffect(() => {
    dispatch(getHouseRule()).then(response => {
      if (response?.error) {
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteHouseRule(deleteStates)).then(response => {
