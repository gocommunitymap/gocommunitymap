...
import { getNav, updateNav, deleteNav } from 'src/store'
import { toast } from 'react-hot-toast'
import { columns } from 'src/views/pages/website/nav/static-data'
...
import { ModalForm } from 'src/views/pages/website/nav/form'
import DeleteModal from 'src/views/components/modal/delete-modal'

const pageTitle = 'Footer Links Setup'
...
const Nav = () => {
  ...
  const store = useSelector(state => state.nav)
  ...
  const [deleteStates, setDeleteStates] = useState({ NAV_ID: null, NAV_DESCRIPTION: '' })
  useEffect(() => {
    dispatch(getNav()).then(response => {
      if (response?.error) {
  ...
  const handleEdit = row => {
    setValue('NAV_ID', row.NAV_ID)
    setValue('NAV_DESCRIPTION', row.NAV_DESCRIPTION)
  ...
  const onSubmit = data => {
    ...
    dispatch(
      updateNav({
        NAV_ID: data.NAV_ID,
        NAV_DESCRIPTION: data.NAV_DESCRIPTION,
        LINK: data.LINK,
  ...
  const handleConfirm = data => {
    ...
    setDeleteStates({ NAV_ID: data.NAV_ID, NAV_DESCRIPTION: data.NAV_DESCRIPTION })
  }

  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteNav(deleteStates)).then(response => {
      ...
      {states.modalForm === 'd' ? (
        ...
        >
          <p style={{ textDecoration: 'underLine', my: 3, textAlign: 'center' }}>
            <strong>
              <span>NAV ID: {deleteStates.NAV_ID}</span>
              <span style={{ paddingLeft: 30 }}>NAV TITLE: {deleteStates.NAV_DESCRIPTION}</span>
            </strong>
          </p>
        </DeleteModal>
      ) : (
  ...
  const exportDataList = dataList?.map(data => ({
    'FOOTER LINK ID': data.NAV_ID,
    DESCRIPTION: data.NAV_DESCRIPTION,
    LINK: data.LINK,
    SORT_ORDER: data.SORT_ORDER,
