...
import { getFeeApply, updateFeeApply, deleteFeeApply } from 'src/store'
import { toast } from 'react-hot-toast'
import { columns } from 'src/views/pages/website/fee-apply/static-data'
...
import { ModalForm } from 'src/views/pages/website/fee-apply/form'
import DeleteModal from 'src/views/components/modal/delete-modal'

const pageTitle = 'Fee Apply Setup'
...
const FeeApply = () => {
  ...
  const store = useSelector(state => state.feeApply)
  ...
  useEffect(() => {
    dispatch(getFeeApply()).then(response => {
      if (response?.error) {
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteFeeApply(deleteStates)).then(response => {
