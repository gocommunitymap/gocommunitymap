...
import { getListingStatus, updateListingStatus, deleteListingStatus } from 'src/store'
import { toast } from 'react-hot-toast'
import { columns } from 'src/views/pages/website/listing-status/static-data'
...
import { ModalForm } from 'src/views/pages/website/listing-status/form'
import DeleteModal from 'src/views/components/modal/delete-modal'

const pageTitle = 'Listing Status'

const defaultValues = {
  ID: null,
  PARAMETER_CODE_2: null,
...
const ListingStatus = () => {
  ...
  const store = useSelector(state => state.listingStatus)
  ...
  useEffect(() => {
    dispatch(getListingStatus()).then(response => {
      if (response?.error) {
  ...
  const handleEdit = row => {
    setValue('ID', row.ID)
    setValue('PARAMETER_CODE_2', { value: row.PARAMETER_CODE_2, label: row.PARAMETER_DESCRIPTION_2 })
  ...
  const onSubmit = data => {
    ...
    dispatch(
      updateListingStatus({
        ID: data.ID,
        PARAMETER_CODE_2: data.PARAMETER_CODE_2.value,
        PARAMETER_DESCRIPTION_2: data.PARAMETER_CODE_2.label,
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteListingStatus(deleteStates)).then(response => {
  ...
  const exportDataList = dataList?.map(data => ({
    TYPE: data.PARAMETER_CODE_2.label,
