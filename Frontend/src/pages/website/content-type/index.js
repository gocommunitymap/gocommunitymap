...
import { getContentType, updateContentType, deleteContentType } from 'src/store'
import { toast } from 'react-hot-toast'
import { columns } from 'src/views/pages/website/content-type/static-data'
...
import { ModalForm } from 'src/views/pages/website/content-type/form'
import DeleteModal from 'src/views/components/modal/delete-modal'

const pageTitle = 'Content Type'
...
const ContentType = () => {
  ...
  const store = useSelector(state => state.contentType)
  ...
  useEffect(() => {
    dispatch(getContentType()).then(response => {
      if (response?.error) {
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteContentType(deleteStates)).then(response => {
