...
import { getUtilityType, updateUtilityType, deleteUtilityType } from 'src/store'
import { toast } from 'react-hot-toast'
import { columns } from 'src/views/pages/website/utility-type/static-data'
...
import { ModalForm } from 'src/views/pages/website/utility-type/form'
import DeleteModal from 'src/views/components/modal/delete-modal'

const pageTitle = 'Utility Types'
...
const UtilityType = () => {
  ...
  const store = useSelector(state => state.utilityType)
  ...
  useEffect(() => {
    dispatch(getUtilityType()).then(response => {
      if (response?.error) {
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteUtilityType(deleteStates)).then(response => {
