...
import { getRoomFaq, updateRoomFaq, deleteRoomFaq } from 'src/store'
...
const RoomFaq = () => {
  ...
  const store = useSelector(state => state.roomFaq)
  ...
  useEffect(() => {
    dispatch(getRoomFaq()).then(response => {
      if (response?.error) {
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteRoomFaq(deleteStates)).then(response => {
