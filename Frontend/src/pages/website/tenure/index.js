...
import { getTenure, updateTenure, deleteTenure } from 'src/store'
import { toast } from 'react-hot-toast'
import { columns } from 'src/views/pages/website/tenure/static-data'
...
import { ModalForm } from 'src/views/pages/website/tenure/form'
import DeleteModal from 'src/views/components/modal/delete-modal'

const pageTitle = 'Tenure'
...
const Tenure = () => {
  ...
  const store = useSelector(state => state.tenure)
  ...
  useEffect(() => {
    dispatch(getTenure()).then(response => {
      if (response?.error) {
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteTenure(deleteStates)).then(response => {
