...
import { getTimeSlot, updateTimeSlot, deleteTimeSlot } from 'src/store'
...
const TimeSlot = () => {
  ...
  useEffect(() => {
    dispatch(getTimeSlot()).then(response => {
      if (response?.error) {
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteTimeSlot(deleteStates)).then(response => {
