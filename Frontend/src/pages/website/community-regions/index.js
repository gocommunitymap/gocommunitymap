...
import { getCommunityRegions, updateCommunityRegions, deleteCommunityRegions } from 'src/store'
import { toast } from 'react-hot-toast'
import { columns } from 'src/views/pages/website/community-regions/static-data'
import { getActiveProps, handleSearch, isAllowed } from 'src/@core/utils'
import { print, generatePDF } from 'src/@core/utils'
import usePermission from 'src/@core/hooks/usePermission'
import { ModalForm } from 'src/views/pages/website/community-regions/form'
import DeleteModal from 'src/views/components/modal/delete-modal'

const pageTitle = 'Community Regions Setup'
...
const CommunityRegions = () => {
  ...
  const store = useSelector(state => state.communityRegions)
  ...
  useEffect(() => {
    dispatch(getCommunityRegions()).then(response => {
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteCommunityRegions(deleteStates)).then(response => {
  ...
  const exportDataList = dataList?.map(data => ({
    REGION_CODE: data.PARAMETER_CODE_3?.toString(),
    REGION_NAME: data.PARAMETER_DESCRIPTION_3,
      ...
      {states.modalForm === 'd' ? (
        ...
        >
          <p style={{ textDecoration: 'underLine', my: 3, textAlign: 'center' }}>
            <strong>
              <span>REGION CODE: {deleteStates.PARAMETER_CODE_3}</span>
              <span style={{ paddingLeft: 30 }}>REGION NAME: {deleteStates.PARAMETER_DESCRIPTION_3}</span>
            </strong>
          </p>
        </DeleteModal>
      ) : (
