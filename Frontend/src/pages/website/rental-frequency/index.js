...
import { getRentalFrequency, updateRentalFrequency, deleteRentalFrequency } from 'src/store'
import { toast } from 'react-hot-toast'
import { columns } from 'src/views/pages/website/rental-frequency/static-data'
...
import { ModalForm } from 'src/views/pages/website/rental-frequency/form'
import DeleteModal from 'src/views/components/modal/delete-modal'

const pageTitle = 'Rental Frequency Setup'
...
const RentalFrequency = () => {
  ...
  const store = useSelector(state => state.rentalFrequency)
  ...
  useEffect(() => {
    dispatch(getRentalFrequency()).then(response => {
      if (response?.error) {
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteRentalFrequency(deleteStates)).then(response => {
...
export default RentalFrequency
