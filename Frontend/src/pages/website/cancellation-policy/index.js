...
import { getCancellationPolicy, updateCancellationPolicy, deleteCancellationPolicy } from 'src/store'
...
const columns = getColumns('Short Badge (e.g. 48h)')
...
const CancellationPolicy = () => {
  ...
  const store = useSelector(state => state.cancellationPolicy)
  ...
  useEffect(() => {
    dispatch(getCancellationPolicy()).then(response => {
      if (response?.error) {
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteCancellationPolicy(deleteStates)).then(response => {
  ...
  const exportDataList = dataList?.map(data => ({
    ...
    'Short Badge': data.PARAMETER_DESCRIPTION_4,
        ...
        <>
          {states.modalForm === 'd' ? (
            ...
            <ModalForm
              ...
              pageTitle={pageTitle}
              hasExtra
              extraLabel='Short Badge (e.g. 48h)'
            />
          )}
        </>
      </Grid>
