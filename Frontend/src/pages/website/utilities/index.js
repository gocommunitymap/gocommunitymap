...
import { getUtilities, updateUtilities, deleteUtilities } from 'src/store'
import { toast } from 'react-hot-toast'
import { columns } from 'src/views/pages/website/utilities/static-data'
...
import { ModalForm } from 'src/views/pages/website/utilities/form'
...
const Utilities = () => {
  ...
  const store = useSelector(state => state.utilities)
  ...
  useEffect(() => {
    dispatch(getUtilities()).then(response => {
      if (response?.error) {
  ...
  const handleEdit = row => {
    setValue('UTILITY_ID ', row.UTILITY_ID)
    setValue('UTILITIES', row.UTILITIES)
    setValue('UTILITY_TYPE_ID', { value: row.UTILITY_TYPE_ID, label: row.UTILITY_TYPE })
  ...
  const onSubmit = data => {
    ...
    dispatch(
      updateUtilities({
        ...
        UTILITY_TYPE_ID: data.UTILITY_TYPE_ID.value,
        ACTIVE: data.ACTIVE
      })
    ).then(response => {
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteUtilities(deleteStates)).then(response => {
      ...
      {states.modalForm === 'd' ? (
        ...
        >
          <p style={{ textDecoration: 'underLine', my: 3, textAlign: 'center' }}>
            <strong>
              <span>UTILITY ID : {deleteStates.UTILITY_ID}</span>
              <span style={{ paddingLeft: 30 }}>UTILITIES: {deleteStates.UTILITIES}</span>
            </strong>
          </p>
        </DeleteModal>
      ) : (
  ...
  const exportDataList = dataList?.map(data => ({
    'UTILITY ID ': data.UTILITY_ID,
    UTILITIES: data.UTILITIES,
    'UTILITIES TYPE': data.UTILITY_TYPE,
