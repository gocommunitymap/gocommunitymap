...
import { getMealPlan, updateMealPlan, deleteMealPlan } from 'src/store'
...
const MealPlan = () => {
  ...
  const store = useSelector(state => state.mealPlan)
  ...
  useEffect(() => {
    dispatch(getMealPlan()).then(response => {
      if (response?.error) {
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteMealPlan(deleteStates)).then(response => {
