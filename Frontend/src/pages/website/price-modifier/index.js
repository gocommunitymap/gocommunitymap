...
import { getPriceModifier, updatePriceModifier, deletePriceModifier } from 'src/store'
import { toast } from 'react-hot-toast'
import { columns } from 'src/views/pages/website/price-modifier/static-data'
...
import { ModalForm } from 'src/views/pages/website/price-modifier/form'
import DeleteModal from 'src/views/components/modal/delete-modal'

const pageTitle = 'Price Modifier'
...
const PriceModifier = () => {
  ...
  const store = useSelector(state => state.priceModifier)
  ...
  useEffect(() => {
    dispatch(getPriceModifier()).then(response => {
      if (response?.error) {
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deletePriceModifier(deleteStates)).then(response => {
