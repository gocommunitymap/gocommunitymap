...
import { getFeatureType, updateFeatureType, deleteFeatureType } from 'src/store'
import { toast } from 'react-hot-toast'
import { columns } from 'src/views/pages/website/feature-type/static-data'
...
import { ModalForm } from 'src/views/pages/website/feature-type/form'
import DeleteModal from 'src/views/components/modal/delete-modal'

const pageTitle = 'Feature Type'
...
const getFlagLabel = value => {
  return toFlagValue(value) ? 'Yes' : 'No'
}
...
const FeatureType = () => {
  ...
  const store = useSelector(state => state.featureType)
  ...
  useEffect(() => {
    dispatch(getFeatureType()).then(response => {
      if (response?.error) {
  ...
  const onSubmit = data => {
    ...
    dispatch(
      updateFeatureType({
        ...
        PARAMETER_CODE_1: hotelsEnabled ? 'Y' : 'N',
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteFeatureType(deleteStates)).then(response => {
