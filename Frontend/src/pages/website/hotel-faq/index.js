...
import { getHotelFaq, updateHotelFaq, deleteHotelFaq } from 'src/store'
...
const HotelFaq = () => {
  ...
  const store = useSelector(state => state.hotelFaq)
  ...
  useEffect(() => {
    dispatch(getHotelFaq()).then(response => {
      if (response?.error) {
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteHotelFaq(deleteStates)).then(response => {
