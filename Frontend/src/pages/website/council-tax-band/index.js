...
import { getCouncilTaxBand, updateCouncilTaxBand, deleteCouncilTaxBand } from 'src/store'
import { toast } from 'react-hot-toast'
import { columns } from 'src/views/pages/website/council-tax-band/static-data'
...
import { ModalForm } from 'src/views/pages/website/council-tax-band/form'
import DeleteModal from 'src/views/components/modal/delete-modal'

const pageTitle = 'Council Tax Band'
...
const CouncilTaxBand = () => {
  ...
  const store = useSelector(state => state.councilTaxBand)
  ...
  useEffect(() => {
    dispatch(getCouncilTaxBand()).then(response => {
      if (response?.error) {
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteCouncilTaxBand(deleteStates)).then(response => {
