...
import { getPropertyFaq, updatePropertyFaq, deletePropertyFaq } from 'src/store'
...
const columns = getColumns('Answer')
...
const PropertyFaq = () => {
  ...
  const store = useSelector(state => state.propertyFaq)
  ...
  useEffect(() => {
    dispatch(getPropertyFaq()).then(response => {
      if (response?.error) {
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deletePropertyFaq(deleteStates)).then(response => {
  ...
  const exportDataList = dataList?.map(data => ({
    ...
    Answer: data.PARAMETER_DESCRIPTION_4,
