...
import { getRoomType, updateRoomType, deleteRoomType } from 'src/store'
...
const RoomType = () => {
  ...
  const store = useSelector(state => state.roomType)
  ...
  useEffect(() => {
    dispatch(getRoomType()).then(response => {
      if (response?.error) {
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteRoomType(deleteStates)).then(response => {
