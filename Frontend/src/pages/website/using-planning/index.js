...
import { getUsingPlanning, updateUsingPlanning, deleteUsingPlanning } from 'src/store'
import { toast } from 'react-hot-toast'
import { columns } from 'src/views/pages/website/using-planning/static-data'
...
import { ModalForm } from 'src/views/pages/website/using-planning/form'
import DeleteModal from 'src/views/components/modal/delete-modal'

const pageTitle = 'Using And Planning'
...
const UsingPlanning = () => {
  ...
  const store = useSelector(state => state.usingPlanning)
  ...
  useEffect(() => {
    dispatch(getUsingPlanning()).then(response => {
      if (response?.error) {
  ...
  const handleEdit = row => {
    setValue('UAP_ID', row.UAP_ID)
    setValue('DESCRIPTION', row.DESCRIPTION)
    setValue('UAP_TYPE_ID', { value: row.UAP_TYPE_ID, label: row.UAP_TYPE_ID })
    setValue('FIELD_TYPE', row.FIELD_TYPE)
    setValue('TOOLTIP_TEXT', row.TOOLTIP_TEXT)
  ...
  const onSubmit = data => {
    ...
    dispatch(
      updateUsingPlanning({
        ...
        UAP_TYPE_ID: data.UAP_TYPE_ID.value,
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteUsingPlanning(deleteStates)).then(response => {
      ...
      {states.modalForm === 'd' ? (
        ...
        >
          <p style={{ textDecoration: 'underLine', my: 3, textAlign: 'center' }}>
            <strong>
              <span>ID: {deleteStates.UAP_ID}</span>
              <span style={{ paddingLeft: 30 }}>DESCRIPTION: {deleteStates.DESCRIPTION}</span>
            </strong>
          </p>
        </DeleteModal>
      ) : (
  ...
  const exportDataList = dataList?.map(data => ({
    ID: data.UAP_ID,
    DESCRIPTION: data.DESCRIPTION,
    TYPE: data.UAP_TYPE_ID,
    'FIELD TYPE': data.UAP_TYPE_ID,
