...
import { getBedType, updateBedType, deleteBedType } from 'src/store'
...
const BedType = () => {
  ...
  const store = useSelector(state => state.bedType)
  ...
  useEffect(() => {
    dispatch(getBedType()).then(response => {
      if (response?.error) {
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteBedType(deleteStates)).then(response => {
