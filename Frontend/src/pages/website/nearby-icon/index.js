...
import { getNearbyIcon, updateNearbyIcon, deleteNearbyIcon } from 'src/store'
...
const NearbyIcon = () => {
  ...
  const store = useSelector(state => state.nearbyIcon)
  ...
  useEffect(() => {
    dispatch(getNearbyIcon()).then(response => {
      if (response?.error) {
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteNearbyIcon(deleteStates)).then(response => {
