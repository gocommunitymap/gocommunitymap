...
import { getFloors, updateFloors, deleteFloors } from 'src/store'
import { toast } from 'react-hot-toast'
import { columns } from 'src/views/pages/website/floors/static-data'
...
import { ModalForm } from 'src/views/pages/website/floors/form'
import DeleteModal from 'src/views/components/modal/delete-modal'

const pageTitle = 'Floors'
...
const Floors = () => {
  ...
  const store = useSelector(state => state.floors)
  ...
  useEffect(() => {
    dispatch(getFloors()).then(response => {
      if (response?.error) {
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteFloors(deleteStates)).then(response => {
