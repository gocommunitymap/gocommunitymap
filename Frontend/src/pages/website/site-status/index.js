...
import { getSiteStatus, updateSiteStatus, deleteSiteStatus } from 'src/store'
import { toast } from 'react-hot-toast'
import { columns } from 'src/views/pages/website/site-status/static-data'
...
import { ModalForm } from 'src/views/pages/website/site-status/form'
import DeleteModal from 'src/views/components/modal/delete-modal'

const pageTitle = 'Site Status'
...
const SiteStatus = () => {
  ...
  const store = useSelector(state => state.siteStatus)
  ...
  useEffect(() => {
    dispatch(getSiteStatus()).then(response => {
      if (response?.error) {
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deleteSiteStatus(deleteStates)).then(response => {
