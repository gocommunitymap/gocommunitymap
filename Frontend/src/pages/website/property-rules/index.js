...
import { getPropertyRules, updatePropertyRules, deletePropertyRules } from 'src/store'
...
const pageTitle = 'Property Rules'
...
const PropertyRules = () => {
  ...
  const store = useSelector(state => state.propertyRules)
  ...
  useEffect(() => {
    dispatch(getPropertyRules()).then(response => {
      if (response?.error) {
  ...
  const onDelete = () => {
    setStates({ ...states, isSubmit: true })
    dispatch(deletePropertyRules(deleteStates)).then(response => {
  ...
  const exportDataList = dataList?.map(data => ({
    ...
    Note: data.PARAMETER_DESCRIPTION_4,
