...
import { axios } from 'src/configs/services/http'
...
const StripePaymentContent = ({ bookingData, selectedRooms = [] }) => {
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      router.push(`/rentals/booking/confirmed?${params.toString()}`)
    } catch (err) {
    ...
    <Grid container spacing={3}>
      <Grid item xs={12} md={7}>
        ...
        {paymentMethod === 'card' && (
          <Card sx={{ p: 3, mb: 3, borderRadius: 2 }}>
            ...
            <FormControlLabel
              ...
              label={<Typography variant='body2'>Same as contact details</Typography>}
            />
        ...
        <Card sx={{ p: 3, mb: 3, borderRadius: 2 }}>
          ...
          <Box sx={{ p: 2, borderRadius: 1.5, backgroundColor: '#f9f9f9', border: '1px solid #e8e8e8', mb: 2 }}>
          ...
          <FormControlLabel
            ...
            label={<Typography variant='body2'>Send me exclusive deals and travel inspiration</Typography>}
          />
        </Card>
...
const RentalBookingPayment = () => {
  ...
  return (
    <>
      <SeoHead title={`Payment – ${themeConfig.templateName}`} description='Complete your rental booking payment.' />
      ...
      <Box sx={{ backgroundColor: '#f8fafc', minHeight: '100vh', pb: 8 }}>
        <Box sx={{ maxWidth: 1100, mx: 'auto', px: { xs: 2, md: 4 }, pt: 4 }}>
          <Elements stripe={stripePromise}>
            <StripePaymentContent bookingData={bookingData} selectedRooms={selectedRooms} />
          </Elements>
        </Box>
      </Box>
    </>
...
RentalBookingPayment.guestGuard = true
RentalBookingPayment.getLayout = page => page

export default RentalBookingPayment
