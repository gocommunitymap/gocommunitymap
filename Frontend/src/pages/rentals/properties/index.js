...
import { getRental } from 'src/store'
...
const defaultValues = {
  ...
  sharedAccommodation: ieoOptions[0].value,
  studentAccepted: ieoOptions[0].value,
  propertyStatus: null,
          ...
          <List sx={{ minWidth: 400 }}>
            ...
            <ListItem>
              <Box sx={{ width: { md: 600, xs: '100%' } }}>
                ...
                <FormGroup>
                  <RadioGroup name='furnished-group' defaultValue={furnishedOptions[0]}>
                    <Grid container spacing={3}>
                      {furnishedOptions.map(item => (
                        <Controller
                          key={item.value}
                          name='furnished'
                          defaultValue={furnishedOptions[0]}
                          control={control}
                          render={({ field: { value, onChange } }) => (
                            <FormControlLabel
                              label={item.label}
...
const Properties = ({ initialProperties = [] }) => {
  ...
  const store = useSelector(state => state.rental)
  ...
  const onSubmit = (data, dataset = []) => {
    ...
    if (data?.sharedAccommodation && data?.sharedAccommodation === 'e') {
      _filteredCount = _filteredCount + 1
      _data = _data.filter(i => i.SHARED_ACCOMMODATION === false)
    } else if (data?.sharedAccommodation && data?.sharedAccommodation === 's') {
      _filteredCount = _filteredCount + 1
      _data = _data.filter(i => i.SHARED_ACCOMMODATION === true)
    }

    if (data?.shortLet && data?.shortLet === 'e') {
      _filteredCount = _filteredCount + 1
      _data = _data.filter(i => i.SHORT_LET === false)
    } else if (data?.shortLet && data?.shortLet === 's') {
      _filteredCount = _filteredCount + 1
      _data = _data.filter(i => i.SHORT_LET === true)
    }

    if (data?.studentAccepted && data?.studentAccepted === 'e') {
      _filteredCount = _filteredCount + 1
      _data = _data.filter(i => i.STUDENT_ACCEPTED === false)
    } else if (data?.studentAccepted && data?.studentAccepted === 's') {
      _filteredCount = _filteredCount + 1
      _data = _data.filter(i => i.STUDENT_ACCEPTED === true)
    }
  ...
  useEffect(() => {
    setIsLoading(true)
    dispatch(getRental({ user: user?.usercode || 0 })).then(response => {
      if (response?.error) {
            ...
            <Grid item xs={12} display={{ md: 'flex', xs: 'block' }} justifyContent='space-between'>
              <Box>
                <Typography variant='h6'>
                  Properties rental <small>{searchStates.formattedAddress?.split(',')[0]}</small>
                </Typography>
                      ...
                      <Grid container>
                        <Grid item xs={12}>
                          {filterList.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map(row => (
                            <PropertyCardRental key={row.PROPERTY_ID} data={row} />
                          ))}
                        </Grid>
                        <Grid item xs={12}>
                          <Card sx={{ p: 2 }}>
...
export const getServerSideProps = async () => {
  try {
    const data = await serverGet(API_URL.GET_PROPERTIES_FULL_DETAILS, { LISTING_TYPE_ID: 2 })
