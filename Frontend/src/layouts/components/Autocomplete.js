...
import { fetchVerticalNav } from 'src/store'
import { API_URL, getNavigationAPI } from 'src/configs'
    ...
    suggestions: [
      {
        ...
        suggestion: 'Properties > Hotels',
        link: 'website/property-setup-hotels/'
      },
      {
        icon: 'tabler:home',
        suggestion: 'Properties > Rentals',
        link: 'website/property-setup-rentals/'
      }
    ]
  }
]
    ...
    <Grid container spacing={6} sx={{ ml: 0 }}>
      {defaultSuggestionsData.map((item, index) => (
        <Grid item xs={12} key={index}>
          <Typography component='p' variant='overline' sx={{ lineHeight: 1.25 }}>
            {item.category}
          </Typography>
          <List sx={{ py: 2.5, width: '100%' }}>
...
const AutocompleteComponent = ({ hidden, settings }) => {
  ...
  useEffect(() => {
    if (user?.usercode) {
      const { usercode, refreshToken } = user
      getNavigationAPI({ usercode, refresh_token: refreshToken })
        .then(response => {
          const searchMenu = response.data?.map(i => ({
            id: i.FORM_ID,
            icon: i.ICON,
            title: i.TITLE,
  ...
  if (!isMounted) {
    ...
    return (
      ...
      >
        <Button color='inherit' sx={!hidden && layout === 'vertical' ? { mr: 1, ml: -2.75 } : {}}>
          <Icon icon='mdi:magnify' />
          <Typography fontSize={9} color='common.white'>
            Search (Ctrl+/)
          </Typography>
        </Button>
        {openDialog && (
