...
const TYPE_CODE = GLOBAL_PARAMETER_TYPES.RECEPTIONS
const ALLOWED = true

export const getReceptions = createAsyncThunk(API_URL.GET_GLOBAL_PARAMETERS, async data => {
  const response = await getGlobalParametersAPI({ ...data, TYPE_CODE, ALLOWED })

  return response?.data
})

export const updateReceptions = createAsyncThunk(
  ...
  async (data, { dispatch, getState }) => {
    ...
    const response = await dispatch(getReceptions(getState().receptions.params))

    return postResponse !== undefined && response.payload
  }
)

export const deleteReceptions = createAsyncThunk(
...
export const receptionsSlice = createSlice({
  name: 'receptions',
  initialState: {
    data: [],
    total: 1,
  ...
  extraReducers: builder => {
    builder.addCase(getReceptions.fulfilled, (state, action) => {
