...
const TYPE_CODE = GLOBAL_PARAMETER_TYPES.ROOM_TYPE
const ALLOWED = true

export const getRoomType = createAsyncThunk(`${API_URL.GET_GLOBAL_PARAMETERS}/roomType`, async data => {
...
export const updateRoomType = createAsyncThunk(
  `${API_URL.CREATE_GLOBAL_PARAMETERS}/roomType`,
  async (data, { dispatch, getState }) => {
    ...
    const response = await dispatch(getRoomType(getState().roomType.params))
...
export const deleteRoomType = createAsyncThunk(
  `${API_URL.DELETE_GLOBAL_PARAMETERS}/roomType`,
  async (data, { dispatch, getState }) => {
...
export const roomTypeSlice = createSlice({
  ...
  extraReducers: builder => {
    builder.addCase(getRoomType.fulfilled, (state, action) => {
