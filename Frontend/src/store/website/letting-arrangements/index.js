...
const TYPE_CODE = GLOBAL_PARAMETER_TYPES.LETTING_ARRANGEMENTS
const ALLOWED = true

export const getLettingArrangements = createAsyncThunk(API_URL.GET_GLOBAL_PARAMETERS, async data => {
  const response = await getGlobalParametersAPI({ ...data, TYPE_CODE, ALLOWED })

  return response?.data
})

export const updateLettingArrangements = createAsyncThunk(
  ...
  async (data, { dispatch, getState }) => {
    ...
    const response = await dispatch(getLettingArrangements(getState().lettingArrangements.params))

    return postResponse !== undefined && response.payload
  }
)

export const deleteLettingArrangements = createAsyncThunk(
...
export const lettingArrangementsSlice = createSlice({
  name: 'lettingArrangements',
  initialState: {
    data: [],
    total: 1,
    params: {},
  ...
  extraReducers: builder => {
    builder.addCase(getLettingArrangements.fulfilled, (state, action) => {
...
export default lettingArrangementsSlice.reducer
