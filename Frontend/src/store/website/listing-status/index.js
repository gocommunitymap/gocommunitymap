...
const TYPE_CODE = GLOBAL_PARAMETER_TYPES.LISTING_TYPE
const ALLOWED = true

export const getListingStatus = createAsyncThunk(API_URL.GET_GLOBAL_PARAMETERS, async data => {
...
export const updateListingStatus = createAsyncThunk(
  ...
  async (data, { dispatch, getState }) => {
    ...
    const response = await dispatch(getListingStatus(getState().listingStatus.params))

    return postResponse !== undefined && response.payload
  }
)

export const deleteListingStatus = createAsyncThunk(
...
export const listingStatusSlice = createSlice({
  name: 'listingStatus',
  initialState: {
    data: [],
    total: 1,
...
export default listingStatusSlice.reducer
