...
const TYPE_CODE = GLOBAL_PARAMETER_TYPES.UNITS
const ALLOWED = true

export const getUnits = createAsyncThunk(API_URL.GET_GLOBAL_PARAMETERS, async data => {
  const response = await getGlobalParametersAPI({ ...data, TYPE_CODE, ALLOWED })

  return response?.data
})

export const updateUnits = createAsyncThunk(API_URL.CREATE_GLOBAL_PARAMETERS, async (data, { dispatch, getState }) => {
  ...
  const response = await dispatch(getUnits(getState().units.params))

  return postResponse !== undefined && response.payload
})

export const deleteUnits = createAsyncThunk(API_URL.DELETE_GLOBAL_PARAMETERS, async (data, { dispatch, getState }) => {
