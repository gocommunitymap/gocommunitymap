...
const TYPE_CODE = GLOBAL_PARAMETER_TYPES.HOTEL_TYPE
const ALLOWED = true

export const getHotelType = createAsyncThunk(`${API_URL.GET_GLOBAL_PARAMETERS}/hotelType`, async data => {
  const response = await getGlobalParametersAPI({ ...data, TYPE_CODE, ALLOWED })

  return response?.data
})

export const updateHotelType = createAsyncThunk(
  `${API_URL.CREATE_GLOBAL_PARAMETERS}/hotelType`,
  async (data, { dispatch, getState }) => {
    ...
    const response = await dispatch(getHotelType(getState().hotelType.params))

    return postResponse !== undefined && response.payload
  }
)

export const deleteHotelType = createAsyncThunk(
  `${API_URL.DELETE_GLOBAL_PARAMETERS}/hotelType`,
  async (data, { dispatch, getState }) => {
...
export const hotelTypeSlice = createSlice({
  ...
  extraReducers: builder => {
    builder.addCase(getHotelType.fulfilled, (state, action) => {
