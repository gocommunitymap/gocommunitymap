...
const TYPE_CODE = GLOBAL_PARAMETER_TYPES.TIME_SLOT
const ALLOWED = true

export const getTimeSlot = createAsyncThunk(`${API_URL.GET_GLOBAL_PARAMETERS}/timeSlot`, async data => {
...
export const updateTimeSlot = createAsyncThunk(
  `${API_URL.CREATE_GLOBAL_PARAMETERS}/timeSlot`,
  async (data, { dispatch, getState }) => {
    ...
    const response = await dispatch(getTimeSlot(getState().timeSlot.params))
...
export const deleteTimeSlot = createAsyncThunk(
  `${API_URL.DELETE_GLOBAL_PARAMETERS}/timeSlot`,
  async (data, { dispatch, getState }) => {
