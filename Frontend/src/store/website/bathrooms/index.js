...
const TYPE_CODE = GLOBAL_PARAMETER_TYPES.BATHROOMS
const ALLOWED = true

export const getBathrooms = createAsyncThunk(API_URL.GET_GLOBAL_PARAMETERS, async data => {
  const response = await getGlobalParametersAPI({ ...data, TYPE_CODE, ALLOWED })

  return response?.data
})

export const updateBathrooms = createAsyncThunk(
  ...
  async (data, { dispatch, getState }) => {
    ...
    const response = await dispatch(getBathrooms(getState().bathrooms.params))

    return postResponse !== undefined && response.payload
  }
)

export const deleteBathrooms = createAsyncThunk(
...
export const bathroomsSlice = createSlice({
  name: 'bathrooms',
  initialState: {
    data: [],
    total: 1,
  ...
  extraReducers: builder => {
    builder.addCase(getBathrooms.fulfilled, (state, action) => {
