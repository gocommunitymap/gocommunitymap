...
const TYPE_CODE = GLOBAL_PARAMETER_TYPES.PRICE_MODIFIER
const ALLOWED = true

export const getPriceModifier = createAsyncThunk(API_URL.GET_GLOBAL_PARAMETERS, async data => {
  const response = await getGlobalParametersAPI({ ...data, TYPE_CODE, ALLOWED })

  return response?.data
})

export const updatePriceModifier = createAsyncThunk(
  ...
  async (data, { dispatch, getState }) => {
    ...
    const response = await dispatch(getPriceModifier(getState().priceModifier.params))

    return postResponse !== undefined && response.payload
  }
)

export const deletePriceModifier = createAsyncThunk(
...
export const priceModifierSlice = createSlice({
  name: 'priceModifier',
  initialState: {
    data: [],
    total: 1,
  ...
  extraReducers: builder => {
    builder.addCase(getPriceModifier.fulfilled, (state, action) => {
...
export default priceModifierSlice.reducer
