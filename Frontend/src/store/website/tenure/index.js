...
const TYPE_CODE = GLOBAL_PARAMETER_TYPES.TENURE
const ALLOWED = true

export const getTenure = createAsyncThunk(API_URL.GET_GLOBAL_PARAMETERS, async data => {
  const response = await getGlobalParametersAPI({ ...data, TYPE_CODE, ALLOWED })

  return response?.data
})

export const updateTenure = createAsyncThunk(API_URL.CREATE_GLOBAL_PARAMETERS, async (data, { dispatch, getState }) => {
  ...
  const response = await dispatch(getTenure(getState().tenure.params))

  return postResponse !== undefined && response.payload
})

export const deleteTenure = createAsyncThunk(API_URL.DELETE_GLOBAL_PARAMETERS, async (data, { dispatch, getState }) => {
...
export const tenureSlice = createSlice({
  ...
  extraReducers: builder => {
    builder.addCase(getTenure.fulfilled, (state, action) => {
