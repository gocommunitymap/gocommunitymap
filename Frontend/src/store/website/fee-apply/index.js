...
const TYPE_CODE = GLOBAL_PARAMETER_TYPES.FEE_APPLY
const ALLOWED = true

export const getFeeApply = createAsyncThunk(API_URL.GET_GLOBAL_PARAMETERS, async data => {
  const response = await getGlobalParametersAPI({ ...data, TYPE_CODE, ALLOWED })

  return response?.data
})

export const updateFeeApply = createAsyncThunk(
  ...
  async (data, { dispatch, getState }) => {
    ...
    const response = await dispatch(getFeeApply(getState().feeApply.params))

    return postResponse !== undefined && response.payload
  }
)

export const deleteFeeApply = createAsyncThunk(
...
export const feeApplySlice = createSlice({
  name: 'feeApply',
  initialState: {
    data: [],
    total: 1,
  ...
  extraReducers: builder => {
    builder.addCase(getFeeApply.fulfilled, (state, action) => {
...
export default feeApplySlice.reducer
