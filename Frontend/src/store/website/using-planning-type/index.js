...
const TYPE_CODE = GLOBAL_PARAMETER_TYPES.USING_AND_PLANNING
const ALLOWED = true

export const getUsingPlanningType = createAsyncThunk(API_URL.GET_GLOBAL_PARAMETERS, async data => {
  const response = await getGlobalParametersAPI({ ...data, TYPE_CODE, ALLOWED })

  return response?.data
})

export const updateUsingPlanningType = createAsyncThunk(
  ...
  async (data, { dispatch, getState }) => {
    ...
    const response = await dispatch(getUsingPlanningType(getState().usingPlanningType.params))

    return postResponse !== undefined && response.payload
  }
)

export const deleteUsingPlanningType = createAsyncThunk(
...
export const usingPlanningTypeSlice = createSlice({
  name: 'usingPlanningType',
  initialState: {
    data: [],
    total: 1,
    params: {},
  ...
  extraReducers: builder => {
    builder.addCase(getUsingPlanningType.fulfilled, (state, action) => {
...
export default usingPlanningTypeSlice.reducer
