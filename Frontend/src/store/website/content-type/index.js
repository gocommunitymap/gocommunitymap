...
const TYPE_CODE = GLOBAL_PARAMETER_TYPES.CONTENT_TYPE
const ALLOWED = true

export const getContentType = createAsyncThunk(API_URL.GET_GLOBAL_PARAMETERS, async data => {
...
export const updateContentType = createAsyncThunk(
  ...
  async (data, { dispatch, getState }) => {
    ...
    const response = await dispatch(getContentType(getState().contentType.params))
...
export const contentTypeSlice = createSlice({
  ...
  extraReducers: builder => {
    builder.addCase(getContentType.fulfilled, (state, action) => {
