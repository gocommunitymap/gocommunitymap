...
const TYPE_CODE = GLOBAL_PARAMETER_TYPES.LETTINGS_DEPOSIT
const ALLOWED = true

export const getLettingsDeposit = createAsyncThunk(API_URL.GET_GLOBAL_PARAMETERS, async data => {
  const response = await getGlobalParametersAPI({ ...data, TYPE_CODE, ALLOWED })

  return response?.data
})

export const updateLettingsDeposit = createAsyncThunk(
  ...
  async (data, { dispatch, getState }) => {
    ...
    const response = await dispatch(getLettingsDeposit(getState().lettingsDeposit.params))

    return postResponse !== undefined && response.payload
  }
)

export const deleteLettingsDeposit = createAsyncThunk(
...
export const lettingsDepositSlice = createSlice({
  name: 'lettingsDeposit',
  initialState: {
    data: [],
    total: 1,
    params: {},
  ...
  extraReducers: builder => {
    builder.addCase(getLettingsDeposit.fulfilled, (state, action) => {
...
export default lettingsDepositSlice.reducer
