...
const TYPE_CODE = GLOBAL_PARAMETER_TYPES.COUNCIL_TAX_BAND
const ALLOWED = true

export const getCouncilTaxBand = createAsyncThunk(API_URL.GET_GLOBAL_PARAMETERS, async data => {
  const response = await getGlobalParametersAPI({ ...data, TYPE_CODE, ALLOWED })

  return response?.data
})

export const updateCouncilTaxBand = createAsyncThunk(
  ...
  async (data, { dispatch, getState }) => {
    ...
    const response = await dispatch(getCouncilTaxBand(getState().councilTaxBand.params))

    return postResponse !== undefined && response.payload
  }
)

export const deleteCouncilTaxBand = createAsyncThunk(
...
export const councilTaxBandSlice = createSlice({
  name: 'councilTaxBand',
  initialState: {
    data: [],
    total: 1,
  ...
  extraReducers: builder => {
    builder.addCase(getCouncilTaxBand.fulfilled, (state, action) => {
...
export default councilTaxBandSlice.reducer
