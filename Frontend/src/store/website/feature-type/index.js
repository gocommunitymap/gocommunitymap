...
const TYPE_CODE = GLOBAL_PARAMETER_TYPES.FEATURE_TYPE
const ALLOWED = true

export const getFeatureType = createAsyncThunk(API_URL.GET_GLOBAL_PARAMETERS, async data => {
...
export const updateFeatureType = createAsyncThunk(
  ...
  async (data, { dispatch, getState }) => {
    ...
    const response = await dispatch(getFeatureType(getState().featureType.params))
...
export const featureTypeSlice = createSlice({
  ...
  extraReducers: builder => {
    builder.addCase(getFeatureType.fulfilled, (state, action) => {
...
export default featureTypeSlice.reducer
