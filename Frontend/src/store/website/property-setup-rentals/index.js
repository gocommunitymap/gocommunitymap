import {
  API_URL,
  getPropertySetupRentalAPI,
  updatePropertySetupRentalAPI,
  createPropertySetupRentalAPI,
...
export const updatePropertySetupRental = createAsyncThunk(
  ...
  async ({ data, files, content_file }, { dispatch, getState }) => {
    ...
    const postResponse = await (isUpdate ? updatePropertySetupRentalAPI(_data) : createPropertySetupRentalAPI(_data))

    await dispatch(getPropertySetupRental(getState().propertySetupRental.params))

    return postResponse !== undefined && postResponse.data
  }
)

export const deletePropertySetupRental = createAsyncThunk(
  ...
  async (data, { dispatch, getState }) => {
    ...
    const postResponse = await deletePropertySetupRentalAPI(data)

    const response = await dispatch(getPropertySetupRental(getState().propertySetupRental.params))

    return postResponse !== undefined && response.payload
  }
)

export const propertySetupRentalSlice = createSlice({
  name: 'propertySetupRental',
  initialState: {
    data: [],
    total: 1,
    params: {},
  ...
  extraReducers: builder => {
    builder.addCase(getPropertySetupRental.fulfilled, (state, action) => {
...
export default propertySetupRentalSlice.reducer
