...
export const ContactNoButton = ({ PROPERTY_ID }) => {
  ...
  const getAgentInfo = async () => {
    setLoading(true)
    await getAgentInfoAPI({ PROPERTY_ID, TYPE: 'C' })
      .then(response => {
        ...
        setShow(true)
        setCount(10)
        setAgentContactNo(response?.data[0].RESULT)
      })
      .catch(() => {
        setShow(false)
      })
  ...
  return show ? (
    <Tooltip title='Click to Copy'>
      <Button loading={loading} onClick={() => handleClipboard(agentContactNo)}>
