...
export const SearchHotelsSectionCard = ({ filterState, setFilterState, setIsFiltered }) => {
  // ** Hook
  ...
  const onSubmit = data => {
    ...
    const _params = jsonToQueryString({
      ...params,
      newHomes: data.newBuildHomesOnly ? 's' : 'i'
    })

    replace(`/hotels/properties${_params}`)
  }
                ...
                <Card sx={{ p: 5 }}>
                  <Grid container spacing={3}>
                    <Grid item xs={12} textAlign='left'>
                      ...
                      <Typography variant='subtitle2'>Search flats and houses hotels in the UK.</Typography>
                    </Grid>
                    <Grid item xs={12} textAlign='left'>
                      ...
                      <Grid container spacing={3}>
                        ...
                        <Grid item xs={12}>
                          <FormControl fullWidth size='small'>
                            <Controller
                              control={control}
                              name='newBuildHomesOnly'
                              render={({ field: { onChange, value } }) => (
                                <FormControlLabel
                                  label='Show new-build homes only'
                                  control={
                                    <Checkbox
                                      color='secondary'
                                      size='medium'
