      ...
      <Typography display='flex' justify-content='left' alignItems='center'>
        ...
        <Tooltip title='View Detail' placement='top'>
          <IconButton size='small' onClick={() => setIsOpen(true)}>
            <img
              src={JSON.parse(row.PICTURE_LINKS)?.find(i => i.IS_PRIMARY).LINK ?? '/images/default/default-picture.png'}
              alt='image'
              style={{
                cursor: 'pointer',
                borderRadius: '50%',
        ...
        <Modal onClose={() => setIsOpen(false)} isOpen={isOpen} title='PICTURE'>
          <Grid container spacing={10}>
            <Grid item xs={12} display='flex' justifyContent='center' flexDirection='column' alignItems='center'>
              <Typography variant='h5'>{row.PROPERTY_NUM_NAME}</Typography>
              <img
                className='rounded me-50'
                src={
                  JSON.parse(row.PICTURE_LINKS)?.find(i => i.IS_PRIMARY).LINK ?? '/images/default/default-picture.png'
                }
                alt='image'
                width='100%'
              />
...
export const columns = [
  ...
  // { flex: 1, headerName: 'PRICE MODIFIER', field: 'PRICE_MODIFIER_ID' },
